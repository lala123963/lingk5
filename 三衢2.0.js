/*
 ...
 功能：完成任务，获得积分
 安装  'crypto' 依赖
 抓包：https://vapp.tmuyun.com/ 任意-请求头中 找到 X-SESSION-ID , X-REQUEST-ID 即可
...
///////////////////////////////////////////////////////////////////
let ckStr = process.env.sanquCookie;
let msg = "";
let ck = "";
...
//显示账号-------------------------------------------------------------------------------------------
!(async () => {
	let ckArr = await getCks(ckStr, "sanquCookie");
	await tips(ckArr);
...
async function user_info() {
	let ts = ts13();
	let _data = `/api/user_mumber/account_detail&&${ck[0]}&&${ck[1]}&&${ts}&&${salt}&&55`;
	let sign = crypto.createHash('sha256').update(_data).digest('hex');
	let options = { 
		method: 'Get', 
...
async function signin() {
	let ts = ts13();
	let _data = `/api/user_mumber/sign&&${ck[0]}&&${ck[1]}&&${ts}&&${salt}&&55`;
	let sign = crypto.createHash('sha256').update(_data).digest('hex');
	let options = { 
		method: 'Get', 
	...
	if (result.code == 0) {
		DoubleLog(`签到: ${result.data.reason} ,获得积分 ${result.data.signIntegral}`);
	} else {
		DoubleLog(`\n 签到失败`);
	}
}
...
async function artic() {
	let ts = ts13();
	let _data = `/api/article/channel_list&&${ck[0]}&&${ck[1]}&&${ts}&&${salt}&&55`;
	let sign = crypto.createHash('sha256').update(_data).digest('hex');
	let options = { 
		method: 'Get', 
		url: `${hostname}/api/article/channel_list?channel_id=63031a10b40eef319070d6b9&isDiFangHao=false&is_new=true&list_count=0&size=20&start=${ts}`, headers: { 
			'X-SESSION-ID': ck[0], 		
			'X-REQUEST-ID': ck[1], 
			'X-TIMESTAMP': ts,
			'X-SIGNATURE': sign, 
		  'X-TENANT-ID': "55", 
			'Cache-Control': " no-cache", 
			'Host': host,
			'User-Agent': '5.2.6;00000000-67d8-c902-ffff-ffffe78175a5;Vivo V1923A;Android;9;Release', 
...
async function read(id) {
	let ts = ts13();
	let _data = `/api/article/detail&&${ck[0]}&&${ck[1]}&&${ts}&&${salt}&&55`;
	let sign = crypto.createHash('sha256').update(_data).digest('hex');
	let options = { 
		method: 'Get', 
	...
	if (result.code == 0) {
		...
		await hot(id);//执行点赞任务
		await wait(5);//等待5秒
		await comment(id);//执行评论任务
		await wait(5);//等待5秒
		await share(3, id);//执行分享任务
	} else {
		DoubleLog(`\n 阅读文章失败 跳过`);
	}
}
...
async function hot(id) {
	let ts = ts13();
	let _data = `/api/favorite/like&&${ck[0]}&&${ck[1]}&&${ts}&&${salt}&&55`;
	let sign = crypto.createHash('sha256').update(_data).digest('hex');
	let options = {
		method: 'POST', 
...
async function comment(id) {
	let ts = ts13();
	let _data = `/api/comment/create&&${ck[0]}&&${ck[1]}&&${ts}&&${salt}&&55`;
	let sign = crypto.createHash('sha256').update(_data).digest('hex');
	let options = {
		method: 'POST', 
...
async function share(n, id) {
	let ts = ts13();
	let _data = `/api/user_mumber/doTask&&${ck[0]}&&${ck[1]}&&${ts}&&${salt}&&55`;
	let sign = crypto.createHash('sha256').update(_data).digest('hex');
	let options = {
		method: 'POST', 
...
async function task() {
	let ts = ts13();
	let _data = `/api/user_mumber/numberCenter&&${ck[0]}&&${ck[1]}&&${ts}&&${salt}&&55`;
	let sign = crypto.createHash('sha256').update(_data).digest('hex');
	let options = { 
		method: 'Get', 
