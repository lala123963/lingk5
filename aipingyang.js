 ...
 */
 ...
 const appid = 35;
 let replytxtArr = ["办好事", "赞", "真好", "厉害", "支持"];
 let cookiesArr = [],
   message = "",
   channelId = [
    "5ed9f49c1b011b069ddcb31d", 
    "5ec2cc34e3ba0f4d91af17bc", 
    "627b0ff4b40eef3d6df33f4f", 
    "627b0fa9de224a04a2797e83", 
    "5ec2cbde1b011b194435b684", 
    "5dbbdb0eb1985007455761f5",
    "5dbbdb441b011b790a33c652"]
 cookie = ($.isNode() ? process.env.pingyck : $.getdata("pingyck")) || ``
 helpAu = ($.isNode() ? process.env.pingyhp : $.getdata("pingyhp")) || true
 //开始===============================================================================================
 !(async () => {
     await requireConfig();//进行cookie处理
     for (let i = 0; i < cookiesArr.length; i++) {
       if (cookiesArr[i]) {
         ...
         $.taskList = {}
         $.queryList = {}
         await getCookie(cookiesArr[i])//进行cookie转换
         await account_detail();//获取账号信息
         console.log(`\n******开始【🐳爱平阳账号${$.index}】${$.nickName}|${$.mobile}*********\n`);
         await main()//调用任务列表
       }
     }
     if ($.isNode() && message) {
 ...
 async function main() {
   if ($.userId) {
    ...
    // await forum()//获取社区文章
     ...
     if (helpAu == true) {
       //console.log(`【环境变量：pingyhp 默认为True 前往助力作者】`)
       await invite()
     }
     console.log(`【查询账号信息】`)
     await account_detail()//账号信息
  ...
  */
  async function credential_auth() {
   let url = {
     ...
     body: `client_id=51&password=${encodeURIComponent($.pwd)}&phone_number=${$.mobile}`,
     headers: {
 ...
 async function comment(id) {
  let body = `channel_article_id=${id}&content=${replytxtArr[Math.floor(Math.random()*replytxtArr.length)]}`
   return new Promise(resolve => {
